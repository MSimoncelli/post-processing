...
for file_inpt_3 in file_list_3:
	layer=file_inpt_3.split('_layer_')[1]
	layer=layer.split('.inpt')[0]
	spc=file_inpt_3.split('_layer_')[0]
	spc=spc.split('spc')[1]
	base=file_inpt_3.split('spc')[0]
	if base not in list_base:
		list_base.append(base)
	dict_files_layer[base,layer,spc]=file_inpt_3

input_list=[]

for base in list_base:
	for layer in ['1','2','3']:
		string_list=[path_inputs_3+'/'+dict_files_layer[base,layer,'1'],path_inputs_3+'/'+dict_files_layer[base,layer,'2']]
		input_list.append(string_list)
for str_list in input_list:
	cmd_list = [startdir+'/scripts/5_conductivity_kubo/msdlayers-mat.x']+str_list
        sp.append(1)
	#print cmd_list
        sp[indx_buf] = subprocess.Popen(cmd_list)
        indx_buf+=1
