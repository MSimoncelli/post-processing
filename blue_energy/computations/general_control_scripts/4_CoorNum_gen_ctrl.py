...
script_folder_1='/4_coord_number' #pay attention at the arguments
script_name_1='/control_COOR_NUM_sp.py' #extracts only the name of the scripta
'''
FULL LIST OF PATHS. 
...
for target_path in paths_to_launch:
	...
	maxrun='%d'%(runmax)
	if ('nacl' in target_path):
		file_input_cutoff='cutoff_dataset_NaCl_1.txt'
	elif('kcl' in target_path):
                file_input_cutoff='cutoff_dataset_KCl_1.txt'
	
	list_arg_script_1=['0', maxrun, file_input_cutoff]
	print 'cd ', dir_to_go
	os.chdir(dir_to_go)
...
for target_path in paths_to_launch:
	name_new_folder=script_folder_1+'_%s_%s_%s'%(list_arg_script_1[0],list_arg_script_1[1],list_arg_script_1[2][:-4])
	cmd='mkdir -p '+common_parent_path+'/analysis/results'+target_path+'/4_coord_num_results_31-05_best'
	os.system(cmd)
	cmd='mv '+common_parent_path+target_path+script_folder_1+'/data_OUT '+common_parent_path+'/analysis/results'+target_path+'/4_coord_num_results_31-05_best/'
	print cmd
	os.system(cmd)
	
	os.chdir(common_parent_path+'/analysis/results'+target_path+'/4_coord_num_results_31-05_best')
	print 'cd ',common_parent_path+'/analysis/results'+target_path+'/4_coord_num_results_31-05_best'
	cmd='mv data_OUT '+name_new_folder[1:]
	print cmd
	os.system(cmd)
