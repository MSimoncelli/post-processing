...
script_folder_1='/1_density' #pay attention at the arguments
script_name_1='/density_AVG.py' #extracts only the name of the script
...
for target_path in paths_to_launch:
	...
	print target_path,runmax
        str_run_max='%d'%(runmax)
	list_arg_script_1=['0', str_run_max]
...
for target_path in paths_to_launch:
	runmax=dict_runmax[target_path]
        print target_path,runmax
        str_run_max_half='%d'%(runmax/2)
        list_arg_script_2=['0', str_run_max_half]
...
for target_path in paths_to_launch:
	runmax=dict_runmax[target_path]
        print target_path,runmax
        str_run_max='%d'%(runmax)
        str_run_max_half='%d'%(runmax/2+1)
        list_arg_script_3=[str_run_max_half,str_run_max]
	#go in the directory where you have just copied the script
        dir_to_go=common_parent_path+target_path+script_folder_1
        print 'cd ', dir_to_go
        os.chdir(dir_to_go)
	list_commands=['python',script_name_1[1:]]
        list_commands+=list_arg_script_3
        #list_commands.append('&')
        print list_commands
        proc_id.append(1)
        proc_id[indx_buf] = subprocess.Popen(list_commands)
...
for target_path in paths_to_launch:
        name_new_folder=script_folder_1+'_results'

        cmd='mv '+common_parent_path+target_path+script_folder_1+'/data_OUT '+common_parent_path+'/analysis/results'+target_path
        print cmd
        os.system(cmd)

        os.chdir(common_parent_path+'/analysis/results'+target_path)
        print 'cd ',common_parent_path+'/analysis/results'+target_path
        cmd='mkdir -p '+name_new_folder[1:] #create folder if not already present
	os.system(cmd)
	cmd='mv ./data_OUT/* .'+name_new_folder
        print cmd
        os.system(cmd)
