...
for target_path in paths_to_launch:
	...
	for irun in range(runmin,runmax+1):
		...
		indx_buf+=1
		cmd_list_1 = ['cp', '/data_prace/michele/prace/cdc/analysis/scripts/3_RDF/calc_RDF_Water_Water.x', dir_to_go]
		sp.append(1)
	...
	for irun in range(runmin,runmax+1):
		...
		print dir_to_go
		sp.append(1)
		sp[indx_buf] = subprocess.Popen(['./calc_RDF_Water_Water.x','%03d'%(irun) ],stdout=subprocess.PIPE)
	...
	for irun in range(runmin,runmax+1):
	        #changes directory
		path_store = '/data_prace/michele/prace/cdc/analysis/results'+target_path+'/3_RDF_water_water'
		cmd = 'mkdir -p '+path_store
		os.system(cmd)
		dir_to_go=common_parent_path+target_path+'/run%03d'%(irun)
                os.chdir(dir_to_go)

		cmd= 'mv run%03dRDF_W_W_for.dat '%(irun)+path_store
		os.system(cmd)

		cmd='rm -r calc_RDF_Water_Water.x'
		os.system(cmd)
		cmd ='rm *.out'
		#print cmd
		os.system(cmd)
