...
histo_PDF_Na={}
norm_Na={}
histo_PDF_Na['pocket']=np.zeros(100)
histo_PDF_Na['edge']=np.zeros(100)
histo_PDF_Na['plane']=np.zeros(100)
histo_PDF_Na['hollow']=np.zeros(100)

histo_PDF_Cl={}
norm_Cl={}
histo_PDF_Cl['pocket']=np.zeros(100)
histo_PDF_Cl['edge']=np.zeros(100)
histo_PDF_Cl['plane']=np.zeros(100)
histo_PDF_Cl['hollow']=np.zeros(100)
coor_x=np.arange(0,100)
...
list_colors={}
list_colors['pocket']='purple'
list_colors['edge']='pink'
list_colors['plane']='#FF8000'
list_colors['hollow']='#FF0000'
...
for datadir in list_sub_dir_here:
	...
	for irun in range(runmin,runmax+1):
		...
		coord= np.loadtxt(data_path,usecols=(8,)) #coordination by O
	...
	for i in range(0,len(histo_PDF_Na['pocket'])):
		if ((histo_PDF_Na['pocket'][i]>0) or(histo_PDF_Na['hollow'][i]>0)) or (histo_PDF_Na['plane'][i]>0):
			j_Na=i
		if ((histo_PDF_Cl['pocket'][i]>0) or(histo_PDF_Cl['hollow'][i]>0)) or (histo_PDF_Cl['plane'][i]>0):
			j_Cl=i
	...
	name_file_save_Na=r'4_solv_C_prob_%s_%s_%03d_%03d_%s_%s_%s_%s' %(region,ion_type_1,runmin,runmax, list_path[8],list_path[9],list_path[10],list_path[11] )
	name_file_save_Na= name_file_save_Na.replace('.','_')
	name_file_save_Na+='.pdf'
	name_file_save_Cl=r'4_solv_C_prob_%s_%s_%03d_%03d_%s_%s_%s_%s' %(region,'Cl',runmin,runmax, list_path[8],list_path[9],list_path[10],list_path[11] )
	name_file_save_Cl= name_file_save_Cl.replace('.','_')
	name_file_save_Cl+='.pdf'
	title_1= r'%s-C %s %s %s %s' %(ion_type_1, list_path[8],spc_title,conC,list_path[11])
	title_1=title_1.replace('_', ' ')
	ax1.set_title(title_1)
	ax1.grid(True)
	ax1.set_xlabel(r'Coordination number of %s by Carbon' %(ion_type_1))
	ax1.set_ylabel(r'Probability')
	title_2= r'%s-C %s %s %s %s' %('Cl', list_path[8],spc_title,conC,list_path[11])
	title_2=title_2.replace('_', ' ')
	ax2.set_title(title_2)
	ax2.grid(True)
	ax2.set_xlabel(r'Coordination number of Cl by Carbon')
	ax2.set_ylabel(r'Probability')
	i=1
	for region in ['edge', 'plane', 'hollow','pocket']:
		ax1.bar(coor_x,histo_PDF_Na[region], label=region+' AVG='+pwu(avg_Na[region],stdev_Na[region]) ,color=list_colors[region],alpha=0.5,align='edge',width=0.3*(-1)**(i))	
		ax1.plot(coor_x,histo_PDF_Na[region],color=list_colors[region])

		ax2.bar(coor_x,histo_PDF_Cl[region], label=region+' AVG='+pwu(avg_Cl[region],stdev_Cl[region]) ,color=list_colors[region], alpha=0.5,align='edge', width=0.3*(-1)**(i) )	
		ax2.plot(coor_x,histo_PDF_Cl[region],color=list_colors[region])
		i+=1
